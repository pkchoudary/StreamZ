...
const SeriesDetail = () => {
  ...
  useEffect(() => {
    const loadAvailabilityByCountry = async () => {
      const availability = await fetchStreamingAvailability(seriesId, selectedCountry, 'tv');
      setPlatformsInCountry(availability);
    };
    loadAvailabilityByCountry();
  }, [seriesId, selectedCountry]);
  ...
  const handlePlatformSelect = async (platformName) => {
    ...
    for (let country of countries) {
      const availability = await fetchStreamingAvailability(seriesId, country.code, 'tv');
